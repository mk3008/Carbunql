...
{
    public static Lex TokenizeAsQueryStart(string sql)
    {
        return TokenizeAsQueryStart(sql.AsMemory());
    }
    ...
    public static Lex TokenizeIdentifier(ReadOnlyMemory<char> memory, int position)
    {
        ...
        if (TryParseLeftParenLex(memory, ref position, out lex)) return lex;
    ...
    private static void SkipWhiteSpaces(ReadOnlyMemory<char> memory, ref int position)
    {
    ...
    private static bool TryParseQueryTerminatorLex(ReadOnlyMemory<char> memory, ref int position, out Lex lex)
    {
        lex = default;
    ...
    [MemberNotNullWhen(true)]
    private static bool TryParseValueSeparatorLex(ReadOnlyMemory<char> memory, ref int position, out Lex lex)
    {
        return TryParseSingleCharLex(memory, ref position, ',', LexType.ValueSeparator, out lex);
    }

    [MemberNotNullWhen(true)]
    private static bool TryParseIdentifierSeparatorLex(ReadOnlyMemory<char> memory, ref int position, out Lex lex)
    {
        return TryParseSingleCharLex(memory, ref position, '.', LexType.IdentifierSeparator, out lex);
    }

    [MemberNotNullWhen(true)]
    private static bool TryParseLeftParenLex(ReadOnlyMemory<char> memory, ref int position, out Lex lex)
    {
